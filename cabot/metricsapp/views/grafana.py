...
from cabot.metricsapp.forms import GrafanaInstanceSelectForm, GrafanaDashboardForm, GrafanaPanelForm, \
    GrafanaSeriesForm
...
class GrafanaInstanceSelectView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if len(instances) == 1:
            ...
            request.session['all_dashboards'] = get_dashboards(instance.url, instance.api_key)
...
class GrafanaDashboardSelectView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid() and not form.errors:
            ...
            dashboard_info = get_dashboard_info(instance.url, instance.api_key, dashboard_uri)
...
class GrafanaSeriesSelectView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        form = self.form_class(request.POST, series=get_series_choices(request.session['dashboard_info'],
                                                                       request.session['panel_info'],
                                                                       request.session['templating_dict']))
        if form.is_valid() and not form.errors:
