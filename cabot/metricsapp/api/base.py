...
def run_metrics_check(check):
    ...
    if series['error'] is True:
        ...
        return StatusCheckResult(status_check=check, succeeded=False, error=error), [check.tag_fetch_error]
    ...
    if series['data'] == []:
        ...
        if check.on_empty_series == defs.ON_EMPTY_SERIES_WARN:
            ...
            return StatusCheckResult(status_check=check, succeeded=False, error='WARNING: no data'), tags
        if check.on_empty_series == defs.ON_EMPTY_SERIES_FAIL:
            check.importance = check.high_alert_importance
            tags = [check.tag_no_data]
            return StatusCheckResult(status_check=check, succeeded=False,
                                     error='{}: no data'.format(check.importance)), tags
