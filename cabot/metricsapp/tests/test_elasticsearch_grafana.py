...
class TestGrafanaTemplating(TestCase):
    def test_templating(self):
        ...
        query = build_query(json.loads(str(templated_panel)), min_time='now-1h')
        validate_query(query)

    def test_auto_time_field(self):
        templates = get_json_file('grafana/templating/auto_time_templating.json')
        panel = get_content('grafana/templating/auto_time_panel.txt')
        templated_panel = template_response(panel, templates)

        created_query = build_query(json.loads(str(templated_panel)), default_interval='2m')
